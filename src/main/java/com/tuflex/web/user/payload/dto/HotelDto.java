...
public class HotelDto {
    ...
    static public HotelDto toDto(Hotel hotel) {
        return new HotelDto(hotel.getPid(), hotel.getCategory(), hotel.getName(),
                hotel.getRegDt().format(DateTimeFormatter.ofPattern("yyyy.MM.dd hh:mm")), hotel.getUrl1(),
                (int) ((hotel.getProducts().get(0).getPrice() + hotel.getProducts().get(1).getPrice()
                        + hotel.getProducts().get(2).getPrice())
                        / hotel.getProducts().stream().filter(p -> p.getIsEnable()).count()),
                String.format("%.1f", hotel.getReviews().size() == 0 ? 0
                        : (double) hotel.getReviews().stream().map(h -> h.getRate()).mapToInt(Integer::intValue).sum()
                                / hotel.getReviews().size()),
                hotel.getLatitude(), hotel.getLongitude(),
                hotel.getProducts().stream().filter(p -> p.getIsEnable()).map(p -> ProductDto.toDto(p)).toList());
    }
}
